...
#'
#' @param .arr1 The first array in the product, if \code{.arr2} has the
#' same size (but not necessarily the same shape), the result will be
#' given in the shape of  \code{.arr1}
#'
#' @param .arr2 The second array in the product.
#'
...
#' product.  If the shape of the resulting array doesn't matter, then
#' the default will avoid an extra permutation at the end of the
#' computation.
#'
#' @return When \code{.arr1} and \code{.arr2} have the same size, the
#' result will be the product of them in the shape of \code{.arr1}.
#' If one of them has fewer dimensions than the other, then recycling
#' will be performed and the result will be given in the shape of the
#' largest one.
#'
#' @export

##  I think this functions should do what I want it to do, but some
